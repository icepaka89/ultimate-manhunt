 ...
 * that dictates if freezing assassins when a speedrunner sees them is enabled.
 * @author : Daniel Gomm
 * @since : 9/1/21, Wed
 */
public class ToggleFreezeAssassinCommandExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(manager.isFreezeAssassinEnabled()) {
            Bukkit.broadcastMessage(ChatColor.AQUA + "Freezing assassins when seen is enabled");
        }
        else {
            Bukkit.broadcastMessage(ChatColor.AQUA + "Freezing assassins when seen is disabled");
        }
